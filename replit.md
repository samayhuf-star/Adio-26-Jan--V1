...
- **Content & Tools**: Ads Search uses a Playwright-based scraper for competitor research. An AI Blog Generator creates long-form content. A Task Manager provides full CRUD operations for projects and tasks with Kanban views. A Blog Section (`/blog`) serves Google Ads articles with category filtering, search, and SEO meta tags. A Google Ads Library (`/blog-2`) serves specialist Google Ads guides across 13 categories (PMax, Smart Bidding, AI Max, Match Types, Shopping, etc.) with IndexNow ping for fast indexing.
...
- **Sitemap & Robots**: `public/sitemap.xml` lists 25+ blog articles and all key pages. The `/sitemap.xml` server route dynamically replaces static blog entries with DB-published posts (falls back to static entries if DB has no published posts, preventing articles from disappearing). Blog-2 posts are added to both sitemaps at `/blog-2/:slug` with `daily` changefreq and `0.8` priority.
- **Google Ads Library** (`/blog-2`): Separate content section for specialist Google Ads guides. Library architecture: `library=null` → main blog (`/blog`), `library='blog-2'` → Google Ads Library (`/blog-2`). SSR handlers at `/blog-2` and `/blog-2/:slug` inject Article JSON-LD + breadcrumb schema. Separate API routes at `/api/blogs-2*`. 13 categories: Intent-Based Campaigns, Geographic Targeting, Match Types, PMax Campaigns, Shopping Campaigns, Smart Bidding, AI Max, Responsive Search Ads, Display Advertising, YouTube Ads, Competitor Targeting, Reporting & Analytics, Campaign Structure.
- **IndexNow**: IndexNow key `e1d486f466a04ef68e517d49595680ff` served at `/e1d486f466a04ef68e517d49595680ff.txt`. `pingIndexNow()` called after each blog-2 article is published to notify Bing/Google for fast indexing. Key file also in `public/`.
