...
- **Campaign Management**: Includes a 7-step Campaign Builder with AI analysis for URL input, various structure selections (SKAG, STAG, Intent-Based, Alpha-Beta), keyword generation, ad generation, geo-targeting, and CSV generation. Saved Campaigns display history and integrate with Google Ads via MCC-based push (REST API v18). Users link their Google Ads Customer ID (no per-user OAuth); campaigns are pushed through the platform's MCC account using `GOOGLE_ADS_MCC_REFRESH_TOKEN` and `GOOGLE_ADS_MCC_CUSTOMER_ID`. Push button enables only after linking. Google Ads push buttons are available to all users (not restricted to superadmin). Billing page shows saved card info (brand, last 4 digits, expiry) via `GET /api/stripe/payment-methods/:email` (auth-protected). **Feature Flag**: Google Ads linking and push UI is controlled by `VITE_GOOGLE_ADS_ENABLED` env var (set to `false` in production, unset in development). Utility: `src/utils/featureFlags.ts`. Affects `CampaignBuilder3.tsx`, `DraftCampaigns.tsx`, `OneClickCampaignBuilder.tsx`, and `ClickGuard.tsx`.
...
- **Unified Users & Billing Tab**: Users, Subscriptions, and Payments are combined into a single "Users & Billing" tab with serial numbers, plan color badges, subscription status, total paid, last payment date/status, and period end columns. Clickable rows open a full User Lifecycle Panel showing the user's complete journey timeline. Data fetched via `GET /api/superadmin/users-unified` (joins users + subscriptions + payments).
- **User Lifecycle Panel**: `src/components/admin/UserLifecyclePanel.tsx` shows a user's full journey when their row is clicked. Displays account age, total payments, emails sent, last login summary cards, and a chronological timeline of all events (signup, login, email sent/opened/clicked, payment, subscription changes, admin actions, errors). Events stored in `user_events` table. Data fetched via `GET /api/superadmin/users/:id/lifecycle`. Event logging integrated into account.ts, stripe.ts, and superadmin.ts routes via `server/services/userEventLogger.ts`.
- **Operational Management**: Features for security (IP blocking, rate limiting), documentation management, campaign template management, website tracking, AI usage tracking, and a full CRUD interface for database administration. Admin can change user passwords via the user edit modal (PUT /api/superadmin/users/:id/password).
