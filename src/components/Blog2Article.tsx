...
import { ChevronLeft, Clock, Calendar, Tag, BookOpen, ArrowRight, Share2, Copy, Check, Zap } from 'lucide-react';
...
export default function Blog2Article({ slug, onBack, onArticleClick }: Blog2ArticleProps) {
  ...
  const fetchArticle = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/blogs-2/${slug}`);
      if (!res.ok) throw new Error('Not found');
      const data = await res.json();
      setArticle(data.article);
      setRelated(data.related || []);
    } catch (err) {
      console.error('Failed to fetch blog-2 article:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(`https://adiology.io/blog-2/${slug}`);
    setCopied(true);
  ...
  return (
    <>
      <Helmet>
        <title>{article.metaTitle || `${article.title} | Google Ads Library — Adiology`}</title>
        <meta name="description" content={article.metaDescription || article.excerpt || ''} />
        <link rel="canonical" href={`https://adiology.io/blog-2/${article.slug}`} />
        <meta property="og:title" content={article.title} />
        <meta property="og:description" content={article.excerpt || ''} />
        <meta property="og:url" content={`https://adiology.io/blog-2/${article.slug}`} />
        ...
        <script type="application/ld+json">{JSON.stringify({
          ...
          "url": `https://adiology.io/blog-2/${article.slug}`,
          "mainEntityOfPage": { "@type": "WebPage", "@id": `https://adiology.io/blog-2/${article.slug}` },
          "articleSection": article.category,
          "keywords": article.tags?.join(', '),
          "wordCount": article.wordCount,
          "timeRequired": article.readTime,
          "inLanguage": "en-US",
          "isPartOf": { "@type": "Blog", "name": "Google Ads Library — Adiology", "url": "https://adiology.io/blog-2" },
          "breadcrumb": {
            "@type": "BreadcrumbList",
            "itemListElement": [
              ...
              { "@type": "ListItem", "position": 3, "name": article.title, "item": `https://adiology.io/blog-2/${article.slug}` }
            ]
          }
        })}</script>
      </Helmet>
        ...
        <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-slate-200/60">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-slate-500">
              ...
              <span className="text-xs font-medium text-blue-600">Adiology</span>
            </div>
            <button
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center gap-3 mt-8 pt-8 border-t border-slate-100">
              <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                {(article.author || 'A').split(' ').map(w => w[0]).join('')}
              </div>
              <div>
